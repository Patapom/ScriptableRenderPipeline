...
void GetSurfaceAndBuiltinData( FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData ) {
	...
	surfaceData.diffuseColor = SAMPLE_TEXTURE2D(_BaseColorMap, sampler_BaseColorMap, baseColorMapUv).rgb * _BaseColor.rgb;
	...
	if ( _DebugMipMapMode != DEBUGMIPMAPMODE_NONE ) {
		surfaceData.diffuseColor = GetTextureDataDebug( _DebugMipMapMode, baseColorMapUv, _BaseColorMap, _BaseColorMap_TexelSize, _BaseColorMap_MipInfo, surfaceData.diffuseColor );
...
//	builtinData.emissiveColor = _EmissiveColor * builtinData.emissiveIntensity * lerp(float3(1.0, 1.0, 1.0), surfaceData.diffuseColor.rgb, _AlbedoAffectEmissive);
